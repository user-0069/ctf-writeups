# No way back home
## Description
>Quantum Computers are going to break some of the standard cryptosystems like RSA, ECC and others. We're in some trouble, so why don't we make our own Quantum-Safe Key Exchange Protocol?
>Challenge files:
> - nowayback.py
> - out.txt
## My solution
```
# Alice side
key = sha256(long_to_bytes(v)).digest()
cipher = AES.new(key, AES.MODE_ECB)
m = pad(FLAG, 16)
c = cipher.encrypt(m).hex()

out = ""
out += f"p, q = ({p}, {q}) \n"
out += f"vka = {vka} \n"
out += f"vkakb = {vkakb} \n"
out += f"vkb = {vkb} \n"
out += f"c = '{c}' \n"
with open("out.txt", "w") as f:
    f.write(out)
```
The goal here is to find `v` to get `sha256` hash and obtain the key, which allows us to decrypt the ciphertext. The output file gives us some infomation: $p, q, v.{k_a}.{k_b},   v.{k_a}, v.{k_b}$ and $c$ in modulo $n$. Normally,
we try `v`= `vka * vkb / vkakb mod n`, but we are stuck at finding the modular inverse of `vkakb` because $gcd(n,v)=p$.

We need a little trick here. As we see in the code `v = (p * randint(1, n)) % n`, so $v = v'.p$. Mathematically, $v'$ has nice properties: $v' \equiv v {mod} q$ and $gcd(v',q) = 1$ at high chance (`randint(1, n) % q==0` almost never happen with true randomness).
So we can all turn the expression from `v` to `v'` and change from `mod n` to `mod q` by dividing by p and taking modulo q. We then now solve `v'` by `v'ka * v'kb / v'kakb mod q`. Subsequently, we obtain `v` = `v' * p`.

Knowing `v`, we can easily get the flag!

```
from Crypto.Cipher import AES
from Crypto.Util.Padding import pad, unpad
from hashlib import sha256
from Crypto.Util.number import getPrime, GCD, bytes_to_long, long_to_bytes, inverse
from random import randint

p, q = (10699940648196411028170713430726559470427113689721202803392638457920771439452897032229838317321639599506283870585924807089941510579727013041135771337631951, 11956676387836512151480744979869173960415735990945471431153245263360714040288733895951317727355037104240049869019766679351362643879028085294045007143623763) 
vka =  
vkakb =  
vkb =  
c = 'fef29e5ff72f28160027959474fc462e2a9e0b2d84b1508f7bd0e270bc98fac942e1402aa12db6e6a36fb380e7b53323' 


v = (vka//p) *(vkb//p) * pow(vkakb//p,-1,q) % q
v=v*p
key = sha256(long_to_bytes(v)).digest()
cipher = AES.new(key, AES.MODE_ECB)
m = cipher.decrypt(bytes.fromhex(c))
print(m)
```
```
b'crypto{1nv3rt1bl3_k3y_3xch4ng3_pr0t0c0l}\x08\x08\x08\x08\x08\x08\x08\x08'
```

